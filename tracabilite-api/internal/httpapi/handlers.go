...
func (h *Handler) Login(c *gin.Context) {
	...
	// Retourner le solde portefeuille pour exportateur / transformateur (crédit démo 2M FCFA).
	walletBalance, walletCreditErr := h.batch.EnsureDemoWalletCredit(ctx, actor.ID, actor.Role)
	resp := gin.H{
		"success":        true,
		"token":          token,
		"actor":          actor,
